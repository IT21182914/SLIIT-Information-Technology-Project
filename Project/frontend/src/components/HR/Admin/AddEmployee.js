...
export default function AddEmployee(onAddEmployee) {
  ...
  function handleUpload() {
    if (true) {
      ...
      if (!phone) {
       alert('Phone number is required with valid format');
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((url) => {
           ...
           alert("Something went wrong fill the form correctly!")
          });
  ...
  const handlePhoneChange = (e) => {
    ...
    if (formattedInput.length === 10) {
      ...
      // If input has more than 10 digits, truncate it to 10 digits
      setPhone(formattedInput);
    } 
  };
